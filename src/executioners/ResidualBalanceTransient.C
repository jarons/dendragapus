...
InputParameters validParams<ResidualBalanceTransient>()
{
  ...
  */
  ...
  //params.addCoupledVar("Other_Residual","name of postprocessor with residual value in it"); 
  params.addRequiredParam<PostprocessorName>("InitialResidual", "The name of the postprocessor you are trying to get.");
  return params;
}
//Because we're inheriting, we can probably eliminate most of these lines...
ResidualBalanceTransient::ResidualBalanceTransient(const std::string & name, InputParameters parameters) :
    ...
    //  _new_tol(getPostprocessorValue("Other_Residual"))  //coupledValue
    _new_tol(getPostprocessorValue("InitialResidual"))  //coupledValue
{
...
void
ResidualBalanceTransient::execute()
{
...
void
ResidualBalanceTransient::computeDT()
{
  _time_stepper->computeStep(); // This is actually when DT gets computed
}

void
ResidualBalanceTransient::incrementStepOrReject()
{
...
ResidualBalanceTransient::solveStep(Real input_dt)
{
  ...
  // new abs_tol used to be here, doesn't work right with timestep_begin multi-apps
  ...
  Real current_norm; // Jaron was here. This affects two lines.
  Real my_current_norm = _problem.computeResidualL2Norm(); // the norm from this app only
  bool _adjust_initial_norm;
  Real his_initial_norm = getPostprocessorValue("InitialResidual");
  if (_picard_max_its > 1)
  {
  ...
  // should this go before Picard stuff?
...
Real
ResidualBalanceTransient::computeConstrainedDT()
{
...
Real
ResidualBalanceTransient::getDT()
{
  return _time_stepper->getCurrentDT();
}

bool
ResidualBalanceTransient::keepGoing()
{
...
void
ResidualBalanceTransient::estimateTimeError()
{
}

bool
ResidualBalanceTransient::lastSolveConverged()
{
  return _time_stepper->converged();
}

void
ResidualBalanceTransient::preExecute()
{
  /*
...
void
ResidualBalanceTransient::postExecute()
{
  _time_stepper->postExecute();
}

Problem &
ResidualBalanceTransient::problem()
{
  return _problem;
}

void
ResidualBalanceTransient::setTargetTime(Real target_time)
{
  _target_time = target_time;
}

Real
ResidualBalanceTransient::getSolutionChangeNorm()
{
  return _solution_change_norm;
}

/*  //We should have inherited this one
void
MethodATransient::setupTimeIntegrator()
{
...
std::string
ResidualBalanceTransient::getTimeStepperName()
{
  if (_time_stepper.get())
