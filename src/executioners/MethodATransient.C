...
template<>
InputParameters validParams<MethodATransient>()
{
...
MethodATransient::MethodATransient(const std::string & name, InputParameters parameters) :
...
MethodATransient::~MethodATransient()
{
}

void
MethodATransient::initialSetup()
{
...
void
MethodATransient::computeDT()
{
  _time_stepper->computeStep(); // This is actually when DT gets computed
}

void
MethodATransient::incrementStepOrReject()
{
...
void
MethodATransient::re_incrementStepOrReject()
{
  if (_last_solve_converged)
  {
#ifdef LIBMESH_ENABLE_AMR
...
void
MethodATransient::takeStep(Real input_dt)
{
  _picard_it = 0;
...
void
MethodATransient::re_takeStep(Real input_dt)
{
  _picard_it = 0;
  while (_picard_it<_picard_max_its && _picard_converged == false)
  {
...
void
MethodATransient::solveStep(Real input_dt)
{
  _dt_old = _dt;
...
void
MethodATransient::re_solveStep(Real input_dt)
{
  _dt_old = _dt;
...
void
MethodATransient::endStep(Real input_time)
{
  if (input_time == -1.0)
    _time = _time_old + _dt;
  else
...
Real
MethodATransient::computeConstrainedDT()
{
...
Real
MethodATransient::getDT()
{
  return _time_stepper->getCurrentDT();
}

bool
MethodATransient::keepGoing()
{
...
void
MethodATransient::estimateTimeError()
{
}

bool
MethodATransient::lastSolveConverged()
{
  return _time_stepper->converged();
}

void
MethodATransient::preExecute()
{
  /*
...
void
MethodATransient::postExecute()
{
  _time_stepper->postExecute();
}

Problem &
MethodATransient::problem()
{
  return _problem;
}

void
MethodATransient::setTargetTime(Real target_time)
{
  _target_time = target_time;
}

Real
MethodATransient::getSolutionChangeNorm()
{
...
std::string
MethodATransient::getTimeStepperName()
{
  if (_time_stepper.get())
