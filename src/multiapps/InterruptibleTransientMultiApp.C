...
template<>
InputParameters validParams<InterruptibleTransientMultiApp>()
{
...
InterruptibleTransientMultiApp::InterruptibleTransientMultiApp(const InputParameters & parameters):
  ...
  //  MultiApp(name, parameters),// should inherit from TransientMultiApp, but I run into problems: _first must be initialized, but it doesn't recognize that the parent takes care of that. Also some problem with the line: InterruptibleTransient * ex = _transient_executioners[i];
    ...
    _my_first(declareRestartableData<bool>("my_first", true)),
    _auto_advance(false),
...
InterruptibleTransientMultiApp::appTransferVector(unsigned int app, std::string var_name)
{
...
void
InterruptibleTransientMultiApp::initialSetup()  
{
...
//_console << "Not dead yet, 1.5" << std::endl;
    ...
    { 
      _re_solved=true; 
      //continue; // removing this line allows re-solving  
    }
...
void
InterruptibleTransientMultiApp::advanceStep()
{
  if (!_auto_advance)
  {
    for (unsigned int i=0; i<_my_num_apps; i++)
...
Real
InterruptibleTransientMultiApp::computeDT()
{
...
void
InterruptibleTransientMultiApp::resetApp(unsigned int global_app, Real /*time*/)  // FIXME: Note that we are passing in time but also grabbing it below
{
...
bool InterruptibleTransientMultiApp::is_re_solved()
{ 
  return _re_solved;
}
