...
template<>
InputParameters validParams<ResidualBalanceMultiApp>()
{
...
ResidualBalanceMultiApp::ResidualBalanceMultiApp(const std::string & name, InputParameters parameters):
...
ResidualBalanceMultiApp::~ResidualBalanceMultiApp()
{
  if (!_has_an_app)
    return;
  ...
  {  // this would not be painful if you used inheritance
...
ResidualBalanceMultiApp::appTransferVector(unsigned int app, std::string var_name)
{
...
void
ResidualBalanceMultiApp::initialSetup()  //used to be init()
{
  MultiApp::initialSetup(); //this line gave an error. I don't remember this line
...
void
ResidualBalanceMultiApp::solveStep(Real dt, Real target_time, bool auto_advance)
{
...
////// This is not necessary, because we are just going to work within the Picard Iteration Framework
//////
    //_re_solved=false;
    if ((ex->getTime() + app_time_offset) + 2e-14 >= target_time) // Maybe this MultiApp was already solved
    { ////dt = 0.0; // This seems to work, but it probably will break transient problems.
      //_re_solved=true;   //alternate method, more reprogramming, dt=0 worked better, or just commenting
      ////app_time_offset -= dt; // does not work: move the multi-app back in time so that it re-solves 
      continue;  // This prevents multiapp re-solve
    }

// plan: set a flag instead of "continue." Use that flag to not advance multi-app in time. 
//       Or step back before stepping forward.
...
//            solution.set(dof, 1);
    ...
    {
      //if (_re_solved)
      //  _console << "RE-Solving Normal Step!" << std::endl;
      //else
        _console << "Solving Normal Step!" << std::endl;
      if (auto_advance)
      { if (_first != true)
        { //_console << "I shot the sheriff" << std::endl;
          //if(_re_solved)
          //  ex->re_incrementStepOrReject();
          //else
            ex->incrementStepOrReject();
      ...
      //  ex->re_takeStep(dt); //we could do it this way because the executioner can't call our methods
                             // I don't know how to pass a pointer to this object to ex
      else
        ex->takeStep(dt);
...
void
ResidualBalanceMultiApp::advanceStep()
{
  if (!_auto_advance)
  {
    for (unsigned int i=0; i<_my_num_apps; i++)
    {
      ...
      ex->endStep();
      ex->incrementStepOrReject();
      //_console << "Jaron, here's your sign" << std::endl;

      // try this and comment previous line. 
      // jk, advanceStep() is not called. 
      /*if (_re_solved)
      {
        _console << "RE-increment" << std::endl; 
        ex->re_incrementStepOrReject();
      }
      else
      {
        _console << "regular increment Step" << std::endl;
        ex->incrementStepOrReject();
      } */
    }
  }
...
Real
ResidualBalanceMultiApp::computeDT()
{
...
void
ResidualBalanceMultiApp::resetApp(unsigned int global_app, Real /*time*/)  // FIXME: Note that we are passing in time but also grabbing it below
{
...
void
ResidualBalanceMultiApp::setupApp(unsigned int i, Real /*time*/)  // FIXME: Should we be passing time?
{
...
//bool ResidualBalanceMultiApp::is_re_solved()
//{ 
//  return _re_solved;
//}
